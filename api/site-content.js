export default async function handler(request, response) {
  ...
  const query = encodeURIComponent(`{\"settings\": *[_type == \"siteSettings\"][0]{companyName,heroTitle,heroText,contactEmail,\"heroImage\":heroImage.asset->url},\"products\": *[_type == \"product\" && published != false]|order(sortOrder asc, sku asc){sku,name,category,brand,fitment,screenSize,summary,features,configurations[]{label,inquiryUrl},\"listImage\":listImage.asset->url,\"detailImage\":detailImage.asset->url}}`)
